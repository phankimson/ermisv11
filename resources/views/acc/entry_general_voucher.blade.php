...
<script>
    jQuery(document).ready(function () {
        ...
        Ermis.columns    = [{"field" :"id", hidden : true},
                            ...
                            { "field" : "debit","title" :"@lang('acc_voucher.debt_account')" ,"url" : "{!!route(env('URL_API').'.acc.'.env('URL_DROPDOWN').'.account-voucher-filter').'?menu='.$menu.'&type=1&option=true'!!}",editor: ItemsReadDropDownEditor , "select" : "OnchangeItem" ,template : "#=getUrlAjaxItemName(debit,'debit')#" ,"width" : "150px" ,"set" : "2" , "key" :true , "field_check" : "subject_debit" },
                            { "field" : "credit","title" :"@lang('acc_voucher.credit_account')" ,"url" : "{!!route(env('URL_API').'.acc.'.env('URL_DROPDOWN').'.account-voucher-filter').'?menu='.$menu.'&type=2&option=true'!!}",editor: ItemsReadDropDownEditor , "select" : "OnchangeItem" ,template : "#=getUrlAjaxItemName(credit,'credit')#" ,"width" : "150px","set" : "2" , "key" :true , "field_check" : "subject_credit" },
                            ...
                            { "field" : "subject_debit","title" : "@lang('acc_voucher.subject_debit')"  ,"url" : "{{route(env('URL_API').'.acc.'.env('URL_DROPDOWN').'.object')}}" ,width : '150px',editor: ItemsReadDropDownEditor ,template : "#=getUrlAjaxItemName(subject_debit,'subject_code','subject_code')#" ,"set" : "2" },                           
                            { "field" : "subject_credit","title" : "@lang('acc_voucher.subject_credit')"  ,"url" : "{{route(env('URL_API').'.acc.'.env('URL_DROPDOWN').'.object')}}" ,width : '150px',editor: ItemsReadDropDownEditor ,template : "#=getUrlAjaxItemName(subject_credit,'subject_code','subject_code')#" ,"set" : "2"},                            
        ...
        Ermis.column_grid = [{ "field" : "id",hidden: true },
                            ...
                            { "field" : "vat_account","title" : "@lang('acc_voucher.vat_account')",width : '200px',"url" : "{{route(env('URL_API').'.acc.'.env('URL_DROPDOWN').'.setting-account-group').'?code=VAT'}}",editor: ItemsReadDropDownEditor ,template : "#=getUrlAjaxItemName(vat_account,'vat_account')#"},
        ...
        Ermis.field = {
            ...
            subject_debit: { field : "subject_debit", defaultValue: DefaultReadValueField() },
            subject_credit: { field : "subject_credit", defaultValue: DefaultReadValueField() },
         ...
         Ermis.field_tax = {
            ...
            description:     {field : "description"},
            vat_account:     {field : "vat_account", defaultValue: RequestURL("{!!route(env('URL_API').'.acc.'.env('URL_DROPDOWN').'.account-voucher-default').'?menu='.$menu.'&type=3'!!}"), validation: { required: true }},
            vat_type:     {field : "vat_type", defaultValue: DefaultReadValueField(), validation: { required: true }},
            vat_tax:     {field : "vat_tax" , defaultValue : 0},
...
<script src="{{ url('addon/scripts/ermis/ermis-form-4-detail-entry.js') }}"></script>
@endsection
