            ...
            @foreach($menu as $m)
              ...
              <h4>{{ $lang == 'vi'? $m->name : $m->name_en }}</h4>
                      @foreach($m->sub_menu as $m1)
                        <li class="{{$m1->id == $menu_current ? 'current_section' : '' }}" mid="{{$m1->id}}" title="{{ $lang == 'vi'? $m1->name : $m1->name_en }}">
                            ...
                            <ul>
                              @foreach($m1->sub_menu1 as $m2)                         
                               <li class="{{$m2->id == $menu_current ? 'current_section' : '' }}" pid ="{{$m2->parent_id}}" title="{{ $lang == 'vi'? $m2->name : $m2->name_en }}">
                      ...
                      @endforeach
                @else
                <li class="{{$m->id == $menu_current ? 'current_section' : '' }}" title="{{ $lang == 'vi'? $m->name : $m->name_en }}">
