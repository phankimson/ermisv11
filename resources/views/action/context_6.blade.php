<ul id="context-menu-grid" style="display:none" aria-hidden="false">
 <li>
     <a class="new_row"><i class="md-icon material-icons" data-uk-tooltip title="@lang('action.new_row')  (Enter)">playlist_add</i>@lang('action.new_row')</a>
 </li>
 <li>
     <a class="close_row"><i class="md-icon material-icons" data-uk-tooltip title="@lang('action.close_row') (Esc)">playlist_play</i>@lang('action.close_row')</a>
 </li>
 <li>
     <a class="remove_row"><i class="md-icon material-icons" data-uk-tooltip title="@lang('action.remove_row') (Del)">line_style</i>@lang('action.remove_row')</a>
 </li>
 <li>
     <a class="remove_all_row"><i class="md-icon material-icons" data-uk-tooltip title="@lang('action.remove_all_row') ({{ config('app.short_key')}}R)">clear_all</i>@lang('action.remove_all_row')</a>
 </li>

</ul>
