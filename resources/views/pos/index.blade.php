  ...
  <div id="sale" class="hide-print flex flex-row h-screen antialiased text-blue-gray-800">
    <div class="flex flex-row w-auto flex-shrink-0 pl-4 pr-2 py-4">
      <div class="flex flex-col items-center py-4 flex-shrink-0 w-20 bg-cyan-500 rounded-3xl">
        <a href="{{url('/pos')}}" class="flex items-center justify-center h-12 w-12 bg-cyan-50 text-cyan-700 rounded-full">
          <img src="{{asset('addon/img/icon.png')}}" alt="logo" height="32" width="32"/>
        </a>
        <ul class="flex flex-col space-y-2 mt-12">
          <li>
            <a href="{{ url('/pos') }}#sale" class="flex items-center" title="Sale" aria-label="Sale" x-on:click.prevent="setTxnType('sale')">
              <span class="flex items-center justify-center h-12 w-12 rounded-2xl text-white" :class="txnType === 'sale' ? 'bg-cyan-300 shadow-lg' : 'text-cyan-100 hover:bg-cyan-400'">
          ...
          <li>
            <a href="{{ url('/pos') }}#return" class="flex items-center" title="Return" aria-label="Return" x-on:click.prevent="setTxnType('return')">
              <span class="flex items-center justify-center h-12 w-12 rounded-2xl text-white" :class="txnType === 'return' ? 'bg-cyan-300 shadow-lg' : 'text-cyan-100 hover:bg-cyan-400'">
          ...
          <li>
            <a href="{{ url('/pos') }}#stock-in" class="flex items-center" title="Stock In" aria-label="Stock In" x-on:click.prevent="setTxnType('stock_in')">
              <span class="flex items-center justify-center h-12 w-12 rounded-2xl" :class="txnType === 'stock_in' ? 'bg-cyan-300 shadow-lg text-white' : 'text-cyan-100 hover:bg-cyan-400'">
          ...
          <li>
            <a href="{{ url('/pos') }}#stock-out" class="flex items-center" title="Stock Out" aria-label="Stock Out" x-on:click.prevent="setTxnType('stock_out')">
              <span class="flex items-center justify-center h-12 w-12 rounded-2xl" :class="txnType === 'stock_out' ? 'bg-cyan-300 shadow-lg text-white' : 'text-cyan-100 hover:bg-cyan-400'">
           ...
           <li>
            <a href="{{ url('/pos') }}#transfer" class="flex items-center" title="Transfer" aria-label="Transfer" x-on:click.prevent="setTxnType('transfer')">
              <span class="flex items-center justify-center h-12 w-12 rounded-2xl" :class="txnType === 'transfer' ? 'bg-cyan-300 shadow-lg text-white' : 'text-cyan-100 hover:bg-cyan-400'">
        ...
        <a href="https://github.com/emsifa/tailwind-pos" target="_blank" class="mt-auto flex items-center justify-center text-cyan-200 hover:text-cyan-100 h-10 w-10 focus:outline-none">
    ...
    <div class="flex-grow flex">
      <div class="flex flex-col bg-blue-gray-50 h-full w-full py-4">
        <div class="flex px-2 flex-row relative">
          ...
          <div class="absolute right-4 top-3 px-4 py-2 rounded-full bg-white shadow text-sm font-semibold text-cyan-700" x-text="currentTxnLabel()"></div>
          <input
            type="text"
      ...
      <div class="w-5/12 flex flex-col bg-blue-gray-50 h-full bg-white pr-4 pl-2 py-4">
        <div class="bg-white rounded-3xl flex flex-col h-full shadow">
          ...
          <div class="select-none h-auto w-full text-center pt-3 pb-4 px-4">
            <div class="mb-3 text-left text-sm text-blue-gray-500">
              Warehouse: <span class="font-semibold text-blue-gray-700" x-text="sourceWarehouseName()"></span>
            </div>
            <div class="mb-3 text-blue-gray-700 px-3 pt-2 pb-3 rounded-lg bg-blue-gray-50">
              <div class="flex text-lg font-semibold">
                <div class="flex-grow text-left">DATE</div>
              </div>
              <input x-model="transactionDate" type="date" class="w-full mt-2 bg-white rounded-lg shadow px-2 py-2 focus:outline-none focus:shadow-lg">
            </div>
            <div x-show="txnType === 'transfer'" class="mb-3 text-blue-gray-700 px-3 pt-2 pb-3 rounded-lg bg-blue-gray-50">
              <div class="flex text-lg font-semibold">
                <div class="flex-grow text-left">WAREHOUSE TO</div>
              </div>
              <select x-model="warehouseToId" class="w-full mt-2 bg-white rounded-lg shadow px-2 py-2 focus:outline-none focus:shadow-lg">
                <option value="">-- SELECT WAREHOUSE --</option>
                <template x-for="warehouse in availableTransferWarehouses()" :key="warehouse.id">
            ...
            <div x-show="txnType === 'sale'" class="mb-3 text-blue-gray-700 px-3 pt-2 pb-3 rounded-lg bg-blue-gray-50">
              <div class="flex text-lg font-semibold">
            ...
            <div x-show="txnType === 'sale' && change > 0" class="flex mb-3 text-lg font-semibold bg-cyan-50 text-blue-gray-700 rounded-lg py-2 px-3">
              <div class="text-cyan-800">CHANGE</div>
              <div class="text-right flex-grow text-cyan-600" x-text="priceFormat(change)"></div>
            </div>
            <div x-show="txnType === 'sale' && change < 0" class="flex mb-3 text-lg font-semibold bg-pink-100 text-blue-gray-700 rounded-lg py-2 px-3">
              <div class="text-right flex-grow text-pink-600" x-text="priceFormat(change)"></div>
            </div>
            <div x-show="txnType === 'sale' && change == 0 && cart.length > 0" class="flex justify-center mb-3 text-lg font-semibold bg-cyan-50 text-cyan-700 rounded-lg py-2 px-3">
              <svg xmlns="http://www.w3.org/2000/svg" class="h-6 w-6 inline-block" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M14 10h4.764a2 2 0 011.789 2.894l-3.5 7A2 2 0 0115.263 21h-4.017c-.163 0-.326-.02-.485-.06L7 20m7-10V5a2 2 0 00-2-2h-.095c-.5 0-.905.405-.905.905 0 .714-.211 1.412-.608 2.006L7 11v9m7-10h-2M7 20H5a2 2 0 01-2-2v-6a2 2 0 012-2h2.5" />
              </svg>
            </div>
            <button class="text-white rounded-2xl text-lg w-full py-3 focus:outline-none" x-bind:class="{ 'bg-cyan-500 hover:bg-cyan-600': submitable(), 'bg-blue-gray-200': !submitable() }" :disabled="!submitable()" x-on:click="submit()" x-text="currentSubmitLabel()">
            </button>
          </div>
        </div>
      </div>
    ...
    <div x-show="isShowModalReceipt" class="fixed w-full h-screen left-0 top-0 z-10 flex flex-wrap justify-center content-center p-24">
      <div x-show="isShowModalReceipt" class="fixed glass w-full h-screen left-0 top-0 z-0" x-on:click="closeModalReceipt()"></div>
      <div x-show="isShowModalReceipt" class="w-96 rounded-3xl bg-white shadow-xl overflow-hidden z-10">
        <div id="receipt-content" class="text-left w-full text-sm p-6 overflow-auto">
          ...
          <div>
            ...
            <div class="text-xs">
              <div class="flex">
                <div class="flex-grow font-semibold">E-INVOICE STATUS</div>
                <div class="font-semibold" x-text="(einvoiceStatus || 'N/A').toUpperCase()"></div>
              </div>
              <div class="mt-1" x-show="einvoiceMessage">
                <span class="font-semibold">MESSAGE:</span>
                <span x-text="einvoiceMessage"></span>
              </div>
              <div class="mt-1" x-show="einvoiceNumber">
                <span class="font-semibold">INVOICE NO:</span>
                <span x-text="einvoiceNumber"></span>
              </div>
              <div class="mt-1" x-show="einvoiceLookupCode">
                <span class="font-semibold">LOOKUP CODE:</span>
                <span x-text="einvoiceLookupCode"></span>
              </div>
              <div class="mt-1 break-all" x-show="einvoiceUrl">
                <span class="font-semibold">INVOICE URL:</span>
                <span x-text="einvoiceUrl"></span>
              </div>
            </div>
          </div>
        </div>
