  ...
  jQuery.each(data, function (k, col) {
      ...
      if (col.key === 'text' || col.key === 'password' || col.key === 'number') {
          if (col.type === 'date') {
            ...
            var get_option = hasWhiteSpace(classes) == true ?  get_option =  classes.split(' ')[1] :  "";
  ...
  jQuery.each(columns, function (k, col) {
      ...
      if (col.key === 'select' && classes == "droplist" ) {       
        ...
        if(jQuery('textarea[name = ' + col.field + ']').hasClass("editor")){
              ...
              jQuery('textarea[name="' + col.field + '"]').val("");
        }
      }else if(col.key == 'text' && col.type == 'string'){
          jQuery('input[name="' + col.field + '"]').val("");
      }
  });
}
...
function calculateTotalRatePaymentAggregate(decimal) {
  ...
  var total = 0;
  for (var i = 0; i < data.length; i++) {
    total += data[i].payment_rate * data[i].rate ;
  }
    return kendo.toString(total, 'n'+decimal);
};
...
function DefaultValueField(){
  ...
  ItemsReadGoodsCritDropDownEditor = function (container, options) {
    var c  = findObjectByKey($kGridTab_column,"field",options.field);
    var crit = (Ermis.voucher.code == "NK" || Ermis.voucher.code == "MH") ?"none" :jQuery('.'+c.crit+'[name="' + c.crit + '"],[name="' + c.crit + '_issue"]').data('kendoDropDownList').value();
    if(crit && crit != "0"){
      jQuery('<input required id="' + options.field + '" data-url="'+c.url+'"  class="dropdown-list-ajax" name="' + options.field + '"/>')
           .appendTo(container)
           .kendoDropDownList({
