...
class AccInventoryTransferImport implements  WithHeadingRow, WithMultipleSheets
{ 
  protected $menu;
  protected $group;
...
class SecondSheetImport implements ToModel, HasReferencesToOtherSheets, WithHeadingRow, WithBatchInserts, WithLimit
{
    public function model(array $row)
    {
        ...
        $credit_import = $credit == null ? $item->stock_account : $credit->id;
        if($item != null && $row['item_code'] && $debit != null && $credit_import != null){             
            ...
            $arr = [
             ...
             $arr_crit = [
                ...
                'general_id'    => $general == null ? 0 : $general->id,
                'detail_id'    => $detail_id,
                'acc_credit'    => $credit == null ? $item->stock_account : $credit->id,
                'acc_debit'    => $debit == null ? 0 : $debit->id,
                'item_id' => $item->id,
                'item_code' => $item->code,
                'item_name' => $item->name,
                'unit' => $item->unit_id,
                'stock_issue' => $stock_issue == null ? $item->stock_default : $stock_issue->id,
                'stock_receipt' => $stock_receipt == null ? '0' : $stock_receipt->id,
        ...
        $data = new AccInventoryTransferImport("","");
