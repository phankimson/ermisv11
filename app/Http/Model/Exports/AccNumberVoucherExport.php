...
class AccNumberVoucherExport implements FromCollection, ShouldAutoSize, WithEvents
{
  protected $select;
  protected $page;
    public function __construct($select,$page)
   {
    ...
    public function collection()
    {
      ...
      $limit = $this->page * (int) config('excel.setting.EXPORT_LIMIT');
        $a = AccNumberVoucher::get_raw_export($this->select,$skip,$limit);
        $b = collect($a);
        if($b->count()>0){
        ...
        $key_trans = $key->map(function ($item, $key) {
          if($item == 'active'){
            ...
            return trans('acc_number_voucher.'.$item);
          }
        });
        $b->prepend($key_trans);
      }
