...
class AccStockBalanceExport implements FromCollection, ShouldAutoSize, WithEvents
{
  protected $select;
  protected $page;
    public function __construct($select,$page)
   {
    ...
    public function collection()
    {
      ...
      $period = 0;
        $a = AccSuppliesGoods::get_raw_balance_export($this->select,$skip,$limit,$period);
        $b = collect($a);
        if($b->count()>0){
        ...
        $key_trans = $key->map(function ($item, $key) {
          ...
          }else if(substr($item,0,-6) == 'quantity' || substr($item,0,-6) == 'amount'){
            return trans('acc_voucher.'.substr($item,0,-6));
          }else{
            return trans('acc_supplies_goods.'.$item);
          }
        });
        $b->prepend($key_trans);
      }
