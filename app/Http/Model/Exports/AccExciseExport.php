...
class AccExciseExport implements FromCollection, ShouldAutoSize, WithEvents
{
  protected $select;
  protected $page;
    public function __construct($select,$page)
   {
    ...
    public function collection()
    {
      $skip = ($this->page - 1) * env("EXPORT_LIMIT");
        ...
        $a = AccExcise::get_raw_export($this->select,$skip,$limit);
        $b = collect($a);
        if($b->count()>0){
