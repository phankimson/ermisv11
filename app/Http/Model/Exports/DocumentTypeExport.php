...
class DocumentTypeExport implements FromCollection, ShouldAutoSize, WithEvents
{
  protected $select;
  protected $page;
    public function __construct($select,$page)
   {
    ...
    public function collection()
    {
      ...
      $limit = $this->page * env("EXPORT_LIMIT");
        $a = DocumentType::get_raw_export($this->select,$skip,$limit);
        $b = collect($a);
        if($b->count()>0){
        ...
        $key_trans = $key->map(function ($item, $key) {
          if($item == 'active'){
            ...
            return trans('document_type.'.$item);
          }
        });
        $b->prepend($key_trans);
      }
