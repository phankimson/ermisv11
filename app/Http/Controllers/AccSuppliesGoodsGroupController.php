...
class AccSuppliesGoodsGroupController extends Controller
{
  ...
  public function __construct(Request $request)
 {
     ...
     $this->key = "supplies-goods-group";
     $this->menu = Menu::where('code', '=', $this->key)->first();
     $this->page_system = "MAX_COUNT_CHANGE_PAGE";   
 }

  public function show(){
    //$data = AccSuppliesGoodsGroup::get_raw();
    $count = AccSuppliesGoodsGroup::count();
    $sys_page = AccSystems::get_systems($this->page_system);
    $paging = $count>$sys_page->value?1:0;   
    return view('acc.supplies_goods_group',['paging' => $paging, 'key' => $this->key ]);
  }

  
  public function data(Request $request){   
    $total = AccSuppliesGoodsGroup::count();
    $sys_page = AccSystems::get_systems($this->page_system);
    $paging = $total>$sys_page->value?1:0;   
    if($paging == 0){
      $arr = AccSuppliesGoodsGroup::get_raw();   
    }else{
    ...
    $asc  = 'desc';
        ...
        if($filter){
          ...
          $arr = AccSuppliesGoodsGroup::get_raw_skip_filter_page($skip,$perPage,$orderby,$asc,$filter_sql);
          $total = AccSuppliesGoodsGroup::whereRaw($filter_sql)->count();
        }else{
          $arr = AccSuppliesGoodsGroup::get_raw_skip_page($skip,$perPage,$orderby,$asc); 
        }   
    }  
  ...
  $validator = Validator::make(collect($arr)->toArray(),[
     ...
     if($validator->passes()){
      $code_check = AccSuppliesGoodsGroup::WhereCheck('code',$arr->code,'id',$arr->id)->first();
      if($code_check == null){
     if($permission['a'] == true && !$arr->id ){
       $type = 2;
       $data = new AccSuppliesGoodsGroup();
       $data->code = $arr->code;
       $data->name = $arr->name;
       ...
       $type = 3;
       $data = AccSuppliesGoodsGroup::find($arr->id);
       // Lưu lịch sử
       $h = new AccHistoryAction();
       $h ->create([
         'type' => $type, // Add : 2 , Edit : 3 , Delete : 4
 ...
 public function DownloadExcel(Request $request){
   return Storage::download('public/downloadFile/AccSuppliesGoodsGroup.xlsx');
 }
 ...
 public function export(Request $request) {
   ...
   try{
       ...
       $myFile = Excel::raw(new AccSuppliesGoodsGroupExport($arr), \Maatwebsite\Excel\Excel::XLSX);
       $response =  array(
         'status' =>true,
         'name' => "AccSuppliesGoodsGroupExportErmis", //no extention needed
