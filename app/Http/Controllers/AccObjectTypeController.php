...
use App\Http\Model\Imports\AccObjectTypeImport;
use App\Http\Model\Exports\AccObjectTypeExport;
use App\Classes\Convert;
use Excel;
use Exception;

class AccObjectTypeController extends Controller
{
  protected $url;
  protected $key;
  protected $menu;
  ...
  public function __construct(Request $request)
 {
  ...
  public function show(){    
    //$data = AccObjectType::get_raw();
    $count = AccObjectType::count();
    $sys_page = AccSystems::get_systems($this->page_system);
    $paging = $count>$sys_page->value?1:0;   
    return view('acc.object_type',['paging' => $paging, 'key' => $this->key ]);
  }

  
  public function data(Request $request){   
    $total = AccObjectType::count();
    $sys_page = AccSystems::get_systems($this->page_system);
    $paging = $total>$sys_page->value?1:0;   
    if($paging == 0){
      $arr = AccObjectType::get_raw();   
    }else{
    ...
    $asc  = 'desc';
        ...
        if($filter){
          ...
          $arr = AccObjectType::get_raw_skip_filter_page($skip,$perPage,$orderby,$asc,$filter_sql);
          $total = AccObjectType::whereRaw($filter_sql)->count();
        }else{
          $arr = AccObjectType::get_raw_skip_page($skip,$perPage,$orderby,$asc); 
        }   
    }  
  ...
  $validator = Validator::make(collect($arr)->toArray(),[
     ...
     if($validator->passes()){
      $code_check = AccObjectType::WhereCheck('code',$arr->code,'id',$arr->id)->first();
      if($code_check == null){
     if($permission['a'] == true && !$arr->id ){
       $type = 2;
       $data = new AccObjectType();
       $data->code = $arr->code;
       $data->name = $arr->name;
       ...
       $type = 3;
       $data = AccObjectType::find($arr->id);
       // Lưu lịch sử
       $h = new AccHistoryAction();
       $h ->create([
         'type' => $type, // Add : 2 , Edit : 3 , Delete : 4
 ...
 public function DownloadExcel(){
   return Storage::download('public/downloadFile/AccObjectType.xlsx');
 }
 ...
 public function export(Request $request) {
   ...
   try{
       ...
       $myFile = Excel::raw(new AccObjectTypeExport($arr), \Maatwebsite\Excel\Excel::XLSX);
       $response =  array(
         'status' =>true,
         'name' => "AccObjectTypeExportErmis", //no extention needed
