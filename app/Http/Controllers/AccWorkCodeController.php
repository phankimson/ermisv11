...
use App\Http\Model\Imports\AccWorkCodeImport;
use App\Http\Model\Exports\AccWorkCodeExport;
use App\Classes\Convert;
use Excel;
use Exception;

class AccWorkCodeController extends Controller
{
  protected $url;
  protected $key;
  protected $menu;
  ...
  public function __construct(Request $request)
 {
  ...
  public function show(){
    //$data = AccWorkCode::get_raw();
    $count = AccWorkCode::count();
    $sys_page = AccSystems::get_systems($this->page_system);
    $paging = $count>$sys_page->value?1:0;
    return view('acc.work_code',['paging' => $paging, 'key' => $this->key ]);
  }

  
  public function data(Request $request){   
    $total = AccWorkCode::count();
    $sys_page = AccSystems::get_systems($this->page_system);
    $paging = $total>$sys_page->value?1:0;   
    if($paging == 0){
      $arr = AccWorkCode::get_raw();   
    }else{
    ...
    $asc  = 'desc';
        ...
        if($filter){
          ...
          $arr = AccWorkCode::get_raw_skip_filter_page($skip,$perPage,$orderby,$asc,$filter_sql);
          $total = AccWorkCode::whereRaw($filter_sql)->count();
        }else{
          $arr = AccWorkCode::get_raw_skip_page($skip,$perPage,$orderby,$asc); 
        }   
    }  
  ...
  $validator = Validator::make(collect($arr)->toArray(),[
     ...
     if($validator->passes()){
      $code_check = AccWorkCode::WhereCheck('code',$arr->code,'id',$arr->id)->first();
      if($code_check == null){
     if($permission['a'] == true && !$arr->id ){
       $type = 2;
       $data = new AccWorkCode();
       $data->code = $arr->code;
       $data->name = $arr->name;
 ...
 public function DownloadExcel(){
   return Storage::download('public/downloadFile/AccWorkCode.xlsx');
 }
