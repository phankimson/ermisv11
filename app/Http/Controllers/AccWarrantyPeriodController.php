...
use App\Http\Model\Imports\AccWarrantyPeriodImport;
use App\Http\Model\Exports\AccWarrantyPeriodExport;
use App\Classes\Convert;
use Excel;
use Exception;

class AccWarrantyPeriodController extends Controller
{
  protected $url;
  protected $key;
  protected $menu;
  ...
  public function __construct(Request $request)
 {
     ...
     $this->key = "warranty-period";
     $this->menu = Menu::where('code', '=', $this->key)->first();
     $this->page_system = "MAX_COUNT_CHANGE_PAGE";   
 }

  public function show(){
    //$data = AccWarrantyPeriod::get_raw();
    $count = AccWarrantyPeriod::count();
    $sys_page = AccSystems::get_systems($this->page_system);
    $paging = $count>$sys_page->value?1:0;  
    return view('acc.warranty_period',['paging' => $paging, 'key' => $this->key ]);
  }

  
  public function data(Request $request){   
    $total = AccWarrantyPeriod::count();
    $sys_page = AccSystems::get_systems($this->page_system);
    $paging = $total>$sys_page->value?1:0;   
    if($paging == 0){
      $arr = AccWarrantyPeriod::get_raw();   
    }else{
    ...
    $asc  = 'desc';
        ...
        if($filter){
          ...
          $arr = AccWarrantyPeriod::get_raw_skip_filter_page($skip,$perPage,$orderby,$asc,$filter_sql);
          $total = AccWarrantyPeriod::whereRaw($filter_sql)->count();
        }else{
          $arr = AccWarrantyPeriod::get_raw_skip_page($skip,$perPage,$orderby,$asc); 
        }   
    }  
  ...
  $validator = Validator::make(collect($arr)->toArray(),[
     ...
     if($validator->passes()){
      $code_check = AccWarrantyPeriod::WhereCheck('code',$arr->code,'id',$arr->id)->first();
      if($code_check == null){
     if($permission['a'] == true && !$arr->id ){
       $type = 2;
       $data = new AccWarrantyPeriod();
       $data->code = $arr->code;
       $data->name = $arr->name;
       ...
       $type = 3;
       $data = AccWarrantyPeriod::find($arr->id);
       // Lưu lịch sử
       $h = new AccHistoryAction();
       $h ->create([
         'type' => $type, // Add : 2 , Edit : 3 , Delete : 4
 ...
 public function DownloadExcel(Request $request){
   return Storage::download('public/downloadFile/AccWarrantyPeriod.xlsx');
 }
 ...
 public function export(Request $request) {
   ...
   try{
       ...
       $myFile = Excel::raw(new AccWarrantyPeriodExport($arr), \Maatwebsite\Excel\Excel::XLSX);
       $response =  array(
         'status' =>true,
         'name' => "AccWarrantyPeriodExportErmis", //no extention needed
