...
class AccObjectGroupController extends Controller
{
  ...
  public function __construct(Request $request)
 {
     ...
     $this->download = 'AccObjectGroup.xlsx';
 }
  ...
  public function data(Request $request){   
    ...
    $asc  = 'desc';
        ...
        if($filter){
          ...
          $arr = AccObjectGroup::get_raw_skip_filter_page($skip,$perPage,$orderby,$asc,$filter_conditions);
          $total = Convert::applyFilterConditions(AccObjectGroup::query(), $filter_conditions)->count();
        }else{
  ...
  public function save(Request $request){
    ...
    try{
     ...
     if($validator->passes()){
      $code_check = AccObjectGroup::WhereCheck('code',$arr->code,'id',$arr->id)->first();
      if($code_check == null){
     if($permission['a'] == true && !$arr->id ){
       $type = 2;
       $data = new AccObjectGroup();
       $data->code = $arr->code;
       $data->name = $arr->name;
       ...
       $type = 3;
       $data = AccObjectGroup::find($arr->id);
       if(!$data){
          return response()->json(['status'=>false,'message'=>trans('messages.no_data_found')]);
        }
       // Luu lich su
  ...
  $type = 5;
   ...
   if($permission['a'] && $request->hasFile('file')){
     ...
     $request->validate([
       ...
       $file = $request->file;
       // Import du lieu
       $import = new AccObjectGroupImport;
 ...
 public function export(Request $request) {
   ...
   try{
    ...
    $page = $request->page;
       ...
       $myFile = Excel::raw(new AccObjectGroupExport($arr,$page), \Maatwebsite\Excel\Excel::XLSX);
       $response =  array(
         'status' =>true,
         'name' => "AccObjectGroupExportErmis", //no extention needed
