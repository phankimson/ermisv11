  ...
  public function __construct(Request $request)
 {
  ...
  public function data(Request $request){   
    ...
    $asc  = 'desc';
        ...
        if($filter){
          ...
          $arr = AccRevenueExpenditure::get_raw_skip_filter_page($skip,$perPage,$orderby,$asc,$filter_conditions);
          $total = Convert::applyFilterConditions(AccRevenueExpenditure::query(), $filter_conditions)->count();
        }else{
  ...
  public function save(Request $request){
    ...
    try{
     ...
     if($validator->passes()){
      $code_check = AccRevenueExpenditure::WhereCheck('code',$arr->code,'id',$arr->id)->first();
      if($code_check == null){
     if($permission['a'] == true && !$arr->id ){
       $type = 2;
       $data = new AccRevenueExpenditure();
       $data->type = $arr->type;
       $data->code = $arr->code;
       $data->name = $arr->name;
       ...
       $type = 3;
       $data = AccRevenueExpenditure::find($arr->id);
       if(!$data){
          return response()->json(['status'=>false,'message'=>trans('messages.no_data_found')]);
        }
       // Luu lich su
  ...
  $type = 5;
   ...
   if($permission['a'] && $request->hasFile('file')){
     ...
     $request->validate([
       ...
       $file = $request->file;
       // Import dữ liệu
       $import = new AccRevenueExpenditureImport;
 ...
 public function export(Request $request) {
   ...
   try{
    ...
    $page = $request->page;
       ...
       $myFile = Excel::raw(new AccRevenueExpenditureExport($arr,$page), \Maatwebsite\Excel\Excel::XLSX);
       $response =  array(
         'status' =>true,
         'name' => "AccRevenueExpenditureExportErmis", //no extention needed
