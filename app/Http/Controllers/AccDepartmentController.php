...
use App\Http\Model\Imports\AccDepartmentImport;
use App\Http\Model\Exports\AccDepartmentExport;
use App\Classes\Convert;
use Maatwebsite\Excel\Facades\Excel;
use Exception;
use Illuminate\Support\Facades\DB;

class AccDepartmentController extends Controller
{
  protected $url;
  protected $key;
  protected $menu;
  ...
  public function show(Request $request){   
    //$data = AccDepartment::get_raw();
    $count = AccDepartment::count();
    $sys_page = AccSystems::get_systems($this->page_system);
    $paging = $count>$sys_page->value?1:0;   
    return view('acc.department',['paging' => $paging, 'key' => $this->key ]);
  }
  ...
  public function save(Request $request){   
    ...
    try{
     ...
     if($validator->passes()){
      $code_check = AccDepartment::WhereCheck('code',$arr->code,'id',$arr->id)->first();
      if($code_check == null){
     if($permission['a'] == true && !$arr->id ){
       $type = 2;
       $data = new AccDepartment();
       $data->code = $arr->code;
       $data->name = $arr->name;
       ...
       $type = 3;
       $data = AccDepartment::find($arr->id);
       // Lưu lịch sử
       $h = new AccHistoryAction();
       $h ->create([
         'type' => $type, // Add : 2 , Edit : 3 , Delete : 4
 ...
 public function DownloadExcel(){
   return Storage::download('public/downloadFile/AccDepartment.xlsx');
 }
 ...
 public function export(Request $request) {   
   ...
   try{
    ...
    $page = $request->page;
       ...
       $myFile = Excel::raw(new AccDepartmentExport($arr,$page), \Maatwebsite\Excel\Excel::XLSX);
       $response =  array(
         'status' =>true,
         'name' => "AccDepartmentExportErmis", //no extention needed
