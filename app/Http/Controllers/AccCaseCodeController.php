...
use App\Http\Model\Imports\AccCaseCodeImport;
use App\Http\Model\Exports\AccCaseCodeExport;
...
class AccCaseCodeController extends Controller
{
  ...
  public function __construct(Request $request)
  {
     ...
     $this->download = 'AccCaseCode.xlsx';
 }

  public function show(){   
    //$data = AccCaseCode::get_raw();
    $count = AccCaseCode::count();
    $sys_page = AccSystems::get_systems($this->page_system);
    $paging = $count>$sys_page->value?1:0;
    return view('acc.case_code',['paging' => $paging, 'key' => $this->key ]);
  }

  public function data(Request $request){   
    $total = AccCaseCode::count();
    $sys_page = AccSystems::get_systems($this->page_system);   
    ...
    $asc  = 'desc';
        ...
        if($filter){
          ...
          $arr = AccCaseCode::get_raw_skip_filter_page($skip,$perPage,$orderby,$asc,$filter_conditions);
          $total = Convert::applyFilterConditions(AccCaseCode::query(), $filter_conditions)->count();
        }else{
          $arr = AccCaseCode::get_raw_skip_page($skip,$perPage,$orderby,$asc); 
        }   
  ...
  public function save(Request $request){
    ...
    try{
     ...
     if($validator->passes()){
      $code_check = AccCaseCode::WhereCheck('code',$arr->code,'id',$arr->id)->first();
      if($code_check == null){
     if($permission['a'] == true && !$arr->id ){
       $type = 2;
       $data = new AccCaseCode();
       $data->code = $arr->code;
       $data->name = $arr->name;
  ...
  $type = 5;
   ...
   if($permission['a'] && $request->hasFile('file')){
     ...
     $request->validate([
       ...
       $file = $request->file;
       // Import du lieu
       $import = new AccCaseCodeImport;
       Excel::import($import, $file);
       // Lay lai du lieu sau khi import
 ...
 public function export(Request $request) {
   ...
   try{
    ...
    $page = $request->page;
       ...
       $myFile = Excel::raw(new AccCaseCodeExport($arr,$page), \Maatwebsite\Excel\Excel::XLSX);
       $response =  array(
         'status' =>true,
         'name' => "AccCaseCodeExportErmis", //no extention needed
