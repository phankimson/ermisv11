...
use App\Http\Model\Imports\AccNumberCodeImport;
use App\Http\Model\Exports\AccNumberCodeExport;
use App\Classes\Convert;
use Excel;
use Exception;

class AccNumberCodeController extends Controller
{
  protected $url;
  protected $key;
  protected $menu;
  ...
  public function data(Request $request){   
    $total = AccNumberCode::count();
    $sys_page = AccSystems::get_systems($this->page_system);
    $paging = $total>$sys_page->value?1:0;   
    if($paging == 0){
      $arr = AccNumberCode::get_raw();   
    }else{
    ...
    $asc  = 'desc';
        ...
        if($filter){
          ...
          $arr = AccNumberCode::get_raw_skip_filter_page($skip,$perPage,$orderby,$asc,$filter_sql);
          $total = AccNumberCode::whereRaw($filter_sql)->count();
        }else{
          $arr = AccNumberCode::get_raw_skip_page($skip,$perPage,$orderby,$asc); 
        }   
    }  
  ...
  $validator = Validator::make(collect($arr)->toArray(),[
     ...
     if($validator->passes()){
      $code_check = AccNumberCode::WhereCheck('code',$arr->code,'id',$arr->id)->first();
      if($code_check == null){
     if($permission['a'] == true && !$arr->id ){
       $type = 2;
       $data = new AccNumberCode();
       ...
       $type = 3;
       $data = AccNumberCode::find($arr->id);
       // Lưu lịch sử
       $h = new AccHistoryAction();
       $h ->create([
         'type' => $type, // Add : 2 , Edit : 3 , Delete : 4
 ...
 public function DownloadExcel(){
   return Storage::download('public/downloadFile/AccNumberCode.xlsx');
 }
  ...
  $type = 5;
   ...
   if($permission['a'] && $request->hasFile('file')){
     ...
     $request->validate([
       ...
       $file = $request->file;
       // Import dữ liệu
       $import = new AccNumberCodeImport;
       Excel::import($import, $file);
       // Lấy lại dữ liệu
       //$array = AccNumberCode::get_raw();
