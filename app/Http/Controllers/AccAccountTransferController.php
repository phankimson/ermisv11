...
use App\Http\Model\Imports\AccAccountTransferImport;
use App\Http\Model\Exports\AccAccountTransferExport;
...
class AccAccountTransferController extends Controller
{
  ...
  public function data(Request $request){   
    $total = AccAccountTransfer::count();
    $sys_page = AccSystems::get_systems($this->page_system);    
  ...
  public function save(Request $request){
    ...
    $validator = Validator::make(collect($arr)->toArray(),[
     ...
     if($validator->passes()){
      $code_check = AccAccountTransfer::WhereCheck('code',$arr->code,'id',$arr->id)->first();
      if($code_check == null){
     if($permission['a'] == true && !$arr->id ){
       $type = 2;
       $data = new AccAccountTransfer();
       $data->code = $arr->code;
       $data->name = $arr->name;
 ...
 public function export(Request $request) {
   ...
   try{
      ...
      $page = $request->page;
       ...
       $myFile = Excel::raw(new AccAccountTransferExport($arr,$page), \Maatwebsite\Excel\Excel::XLSX);
       $response =  array(
         'status' =>true,
         'name' => "AccAccountTransferExportErmis", //no extention needed
