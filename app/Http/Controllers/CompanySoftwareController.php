...
class CompanySoftwareController extends Controller
{
  ...
  protected $download;
    public function __construct(Request $request)
   {
       ...
       $this->download = "CompanySoftware.xlsx"; // File download name
   }
   ...
   public function show(){
      ...
      return view('manage.'.str_replace("-", "_", $this->key),['paging' => $paging, 'software'=>$software, 'key' => $this->key , 'type' => $type->id]);
   }
   ...
   public function data(Request $request){    
    ...
    $asc  = 'desc';
        ...
        if($filter){
          ...
          $arr = CompanySoftware::get_raw_skip_filter_page($skip,$perPage,$orderby,$asc,$filter_conditions,$type->id);
          $total = Convert::applyFilterConditions(CompanySoftware::query(), $filter_conditions)->count();
        }else{
        ...
        if(!$db_temp){
          ...
          if($db_temp){
            ...
            if ($db_check == 0) {
              ...
              $data->save();
              // LÃƒâ€ Ã‚Â°u lÃƒÂ¡Ã‚Â»Ã¢â‚¬Â¹ch sÃƒÂ¡Ã‚Â»Ã‚Â­ ---- NOT EDIT
              $h = new HistoryAction();
              $h ->create([
  ...
  public function export(Request $request) {
    ...
    try{
        ...
        $myFile = Excel::raw(new CompanySoftwareExport($arr,$page), \Maatwebsite\Excel\Excel::XLSX);
        $response =  array(
          'status' =>true,
