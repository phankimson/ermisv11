...
class AccAccountTypeController extends Controller
{
  ...
  public function show(){
    ...
    $count = AccAccountType::count();
    $sys_page = AccSystems::get_systems($this->page_system);
    $paging = $count>$sys_page->value?1:0;   
    return view('acc.account_type',['paging' => $paging, 'key' => $this->key ]);
  }


  
  public function data(Request $request){   
    $total = AccAccountType::count();
    $sys_page = AccSystems::get_systems($this->page_system);
    $paging = $total>$sys_page->value?1:0;   
    if($paging == 0){
      $arr = AccAccountType::get_raw();   
    }else{
    ...
    $asc  = 'desc';
        ...
        if($filter){
          ...
          $arr = AccAccountType::get_raw_skip_filter_page($skip,$perPage,$orderby,$asc,$filter_sql);
          $total = AccAccountType::whereRaw($filter_sql)->count();
        }else{
          $arr = AccAccountType::get_raw_skip_page($skip,$perPage,$orderby,$asc); 
        }   
    }  
  ...
  $validator = Validator::make(collect($arr)->toArray(),[
     ...
     if($validator->passes()){
      $code_check = AccAccountType::WhereCheck('code',$arr->code,'id',$arr->id)->first();
      if($code_check == null){
     if($permission['a'] == true && !$arr->id ){
       $type = 2;
       $data = new AccAccountType();
       $data->code = $arr->code;
       $data->name = $arr->name;
       ...
       $type = 3;
       $data = AccAccountType::find($arr->id);
       // Lưu lịch sử
       $h = new AccHistoryAction();
       $h ->create([
         'type' => $type, // Add : 2 , Edit : 3 , Delete : 4
 ...
 public function DownloadExcel(){
   return Storage::download('public/downloadFile/AccAccountType.xlsx');
 }
 ...
 public function export(Request $request) {
   ...
   try{
       ...
       $myFile = Excel::raw(new AccAccountTypeExport($arr), \Maatwebsite\Excel\Excel::XLSX);
       $response =  array(
         'status' =>true,
         'name' => "AccAccountTypeExportErmis", //no extention needed
