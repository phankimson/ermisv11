          ...
          // CHI TIET / Detail
           foreach($arr->detail as $k => $d){
            ...
            if($d->detail_id){
             ...
             $detail->debit = $setting_voucher->debit;  // Lay tu seting default
             $detail->credit = $setting_voucher->credit; // Lay tu seting default
             ...
             if(($vat->payment == 1 && !$d->id)){              
              ...
              if($d->id){
                ...
                // Ktra xem chinh sua tt du chua. Neu chinh sua ma payment = 1 (da thanh toan) thi rollback.             
                ...
                if($vat_payment_id+(float)$d->payment <= (float)$vat->total_amount){
                  ...
                  $tax_payment->save(); 
                   ...
                   // Update lai so tien da thanh toan cua tung phieu khac (truong hop chinh sua phieu da thanh toan, nen so tien da thanh toan cua phieu do se giam, va so tien con lai se tang len, va nguoc lai)
             ...
             if($setting_voucher->credit){
              $balance = $this->reduceCurrency($setting_voucher->credit,$arr->currency,$d->payment,$d->rate);
