...
class AccSettingAccountGroupController extends Controller
{
  public function __construct(Request $request)
 {
  ...
  $validator = Validator::make(collect($arr)->toArray(),[
     ...
     if($validator->passes()){
      $code_check = AccSettingAccountGroup::WhereCheck('code',$arr->code,'id',$arr->id)->first();
      if($code_check == null){
     if($permission['a'] == true && !$arr->id ){
       $type = 2;
       $data = new AccSettingAccountGroup();
       $data->code = $arr->code;
       $data->name = $arr->name;
      ...
      foreach($arr->account_filter as $t){
       ...
       $type = 3;
       $data = AccSettingAccountGroup::find($arr->id);
       // Lưu lịch sử
       $h = new AccHistoryAction();
       $h ->create([
         'type' => $type, // Add : 2 , Edit : 3 , Delete : 4
      ...
      // Xoa Setting Account Group Filter 
      if($ob_all->count()>0){
  ...
  $type = 5;
   ...
   if($permission['a'] && $request->hasFile('file')){
     ...
     $request->validate([
       ...
       $file = $request->file;
       // Import dữ liệu
       Excel::import(new AccSettingAccountGroupImport, $file);
       // Lấy lại dữ liệu
       $array = AccSettingAccountGroup::get_raw();
 ...
 public function export(Request $request) {
   ...
   try{
       ...
       $myFile = Excel::raw(new AccSettingAccountGroupExport($arr), \Maatwebsite\Excel\Excel::XLSX);
       $response =  array(
         'status' =>true,
         'name' => "AccSettingAccountGroupExportErmis", //no extention needed
