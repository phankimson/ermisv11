...
{
  use CurrencyCheckTraits,FileAttachTraits,NumberVoucherTraits,AccHistoryTraits,ReferenceTraits;
  protected $url;
  protected $key;
  protected $menu;
  protected $group;
         ...
         $this->saveReference($arr->reference_by,$general->id);
          ...
          // CHI TIET / Detail
           foreach($arr->detail as $k => $d){
             ...
             // Luu so ton ben No
             if(substr($d->debit->text,0,3) === '111' || substr($d->debit->text,0,3) === '113'){ 
